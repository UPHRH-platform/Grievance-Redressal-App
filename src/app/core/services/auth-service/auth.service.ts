...
export class AuthService extends HttpService{
  ...
  getUserRoles(): string[] {
    ...
    if (token) {
      const userData= this.getUserData();
      const userRole = userData.userRepresentation?.attributes?.role ? userData.userRepresentation.attributes.role[0] : userData.userRepresentation.attributes.Role[0];
      switch(userRole) {
        case 'SUPERADMIN':
